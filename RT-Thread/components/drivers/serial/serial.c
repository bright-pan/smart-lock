...
{
    rt_memset(rbuffer->buffer, 0, sizeof(rbuffer->size));
    rbuffer->put_index = 0;
    rbuffer->get_index = 0;
}
                                      ...
                                      char                      ch)
{
    ...
    rbuffer->put_index = (rbuffer->put_index + 1) & (rbuffer->size - 1);

    /* if the next position is read index, discard this 'read char' */
    if (rbuffer->put_index == rbuffer->get_index)
    {
        rbuffer->get_index = (rbuffer->get_index + 1) & (rbuffer->size - 1);
    }
                                        ...
                                        char                      ch)
{
    ...
    next_index = (rbuffer->put_index + 1) & (rbuffer->size - 1);
...
rt_inline rt_uint32_t serial_ringbuffer_size(struct serial_ringbuffer *rbuffer)
{
    ...
    level = rt_hw_interrupt_disable();
    size = (rbuffer->put_index - rbuffer->get_index) & (rbuffer->size - 1);
    rt_hw_interrupt_enable(level);
